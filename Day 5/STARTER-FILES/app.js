const episodes = [
  ...
  const ulListElement = document.querySelector("ul.episodes")
  
  let lastChecked = null

  const displayEpisodes = () => {
    for (const [index, episodeObj] of episodes.entries()) {
      ...
      labelElement.htmlFor = number
      inputElement.type = "checkbox"
      inputElement.name = number
      inputElement.id = number
      inputElement.setAttribute("data-index", index)
      spanElement.textContent = "" + (index+1) + " || " + episodeObj.name

      labelElement.appendChild(inputElement)
      labelElement.appendChild(spanElement)
      listEntry.appendChild(labelElement)

      ulListElement.appendChild(listEntry)
    }
  }
  ...
  for (const input of entries) {
    input.addEventListener("click", (event) => {
      ...
      if (lastChecked && event.shiftKey) {
        let currentCheckedIndex = parseInt(input.getAttribute("data-index"))
        let lastCheckedIndex = parseInt(lastChecked.getAttribute("data-index"))
        let check_or_uncheck = input.checked

        let indices = [lastCheckedIndex, currentCheckedIndex]
        if (currentCheckedIndex < lastCheckedIndex) {
          indices = [currentCheckedIndex, lastCheckedIndex]
        }
        for (let index=indices[0]; index<=indices[1]; index++) {
          entries[index].checked = check_or_uncheck
        }
      }
      lastChecked = input
    })
