...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const displayedImage = document.querySelector(".feature > img")
  const displayedCaption = document.querySelector(".feature > div.caption")

  let currentSelected

  const displayNewImageAndCaption = (newIndex) => {
    let imageName = listItems[newIndex].querySelector("img").src
    imageName = imageName.substring(imageName.lastIndexOf("/") + 1)
    let contentIndex = getContentIndex(imageName)
    displayedImage.src = "./images/" + content[contentIndex].image
    displayedCaption.textContent = content[contentIndex].caption
  }

  rightButton.addEventListener("click", () => {
    getCurrentIndex()
    listItems[currentSelected].classList.remove("selected")
    let newIndex = (currentSelected === listItems.length-1) ? 0 : currentSelected+1
    listItems[newIndex].classList.add("selected")

    displayNewImageAndCaption(newIndex)
  })

  leftButton.addEventListener("click", () => {
    getCurrentIndex()
    listItems[currentSelected].classList.remove("selected")
    let newIndex = (currentSelected === 0) ? listItems.length-1 : currentSelected-1
    listItems[newIndex].classList.add("selected")
  ...
  for (const [index, listItem] of listItems.entries()) {
    listItem.addEventListener("click", () => {
      ...
      displayNewImageAndCaption(index)
    })
  }
...
const getContentIndex = (imagePath) => {
  for (const [index, object] of Object.entries(content)) {
    if (object.image === imagePath) {
      return index
    }
  }
}