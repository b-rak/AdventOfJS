...
document.addEventListener("DOMContentLoaded", () => {
  const episodeElements = document.querySelectorAll("div.episode")
  ...
  const mainDescription = document.querySelector(".content > p")
  const mainMoreLink = document.querySelector(".content > a")

  for (let i=0; i<episodeElements.length; i++) {
    episodeElements[i].textContent = "Episode " + episodes[i]["id"]
    titleElements[i].textContent = episodes[i]["title"]
  }
  ...
  for (let [index, listitem] of listItems.entries()) {
    listitem.addEventListener("click", () => {
      ...
      imgElement.src = "./images/" + episodes[index]["cover"]
      imgElement.alt = "Episode " + episodes[index]["id"]
      mainTitle.textContent = episodes[index]["title"]
      mainDescription.textContent = episodes[index]["description"]
      mainMoreLink.href = episodes[index]["link"]
    })
  }
})